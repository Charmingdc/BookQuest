...
const fetchRecommendedBooks = async (genres: string[]): Promise<Book[]> => {
  ...
  const response = await fetch(
    `https://openlibrary.org/search.json?q=${genreQuery}&fields=key,title,author_name,first_publish_year,cover_i,ratings_average,edition_count,author_key,subject,isbn&sort=random&limit=50`
  );
